...
Events.prototype.on = function (event, callback) {
    ...
    callback = callback.bind(this); // bind the function the loan object
...
Events.prototype.trigger = function (event) {
    ...
    if (this.callbacks && this.callbacks[event]) {
        ...
        // if an arrow function is used for the forEach callback below,
...
Events.prototype.off = function (event, callback) {
    ...
    if (this.callbacks[event]) {
        ...
        for (let i = 0, len = this.callbacks[event].length; i < len; i += 1) {
